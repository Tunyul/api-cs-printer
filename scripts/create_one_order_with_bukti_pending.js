// scripts/create_one_order_with_bukti_pending.js
// Run with: node scripts/create_one_order_with_bukti_pending.js
...
async function ensureCustomer() {
  ...
  if (c) return c;
  return models.Customer.create({ nama: 'Pending Bukti Customer', no_hp: '6287700000002', tipe_customer: 'reguler', batas_piutang: null, catatan: '', created_at: new Date(), updated_at: new Date() });
}

async function ensureProduct() {
  const p = await models.Product.findOne();
  if (p) return p;
  return models.Product.create({ kategori: 'Cetak', nama_produk: 'Produk Pending Bukti', bahan: 'Karton', finishing: 'Gloss', ukuran_standar: 'pcs', harga_per_pcs: 120000, unit_area: null, waktu_proses: '1 hari', created_at: new Date(), updated_at: new Date(), stock: 5 });
}
...
async function main() {
  try {
    ...
    await models.sequelize.transaction(async (t) => {
      ...
      const totalBayar = 120000; // default total for the order
      ...
      // create payment with bukti URL and status 'menunggu_verifikasi', nominal 0
      const payment = await models.Payment.create({ id_order: order.id_order, id_customer: order.id_customer, no_transaksi: order.no_transaksi, no_hp: customer.no_hp, nominal: 0, bukti: buktiUrl, tipe: 'dp', status: 'menunggu_verifikasi', tanggal: new Date(), created_at: new Date(), updated_at: new Date() }, { transaction: t });

      created = { no_transaksi: order.no_transaksi, id_order: order.id_order, id_payment: payment.id_payment };
    });

    console.log('Created order with pending bukti payment:', created);
    process.exit(0);
  } catch (err) {
    console.error('Error creating pending bukti payment order:', err);
    process.exit(1);
  }
}
